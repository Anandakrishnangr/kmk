...
function SellerSignup() {
    ...
    const init = <form onSubmit={handleSubmit} method="POST" className="justify-content-center w-100">
        ...
        <input type="submit" onClick={() => dispatch('next')} className="btn mt-3 btn-warning form-control" value="Next"></input>
    ...
    const reducer = (state, action) => {
        switch (action) {
            case 'prev': return init;

            case 'next':
                return <form onSubmit={handleSubmit} method="POST" className="justify-content-center w-100">
                    <h3>Create account</h3>
                    <div>
                        Company name<br></br>
                        <input className="form-control" onChange={(e) => setusername(e.target.value)} type="text" name="usname"></input>
                    ...
                    <div>
                        Company/Shop Registeration Licence<br></br>
                        <input className="form-control" onChange={(e) => setemail(e.target.value)} type="file" name="email" min="10" max="10"></input>
                        <span className="ms-1 h-15"><i className="fa fa-info text-danger" aria-hidden="true"></i> </span>

                    </div>
                    <div>
                        Address<br></br>
                        <textarea className="form-control" onChange={(e) => setpass1(e.target.value)}  type="password" name="pass1"></textarea>
                        <span className="ms-1 h-15 text-info"><i className="fa fa-info text-info" aria-hidden="true"></i> Password must be at least 6 characters.</span>
                    </div>
                    <div>
                        State<br></br>
                    ...
                    <div className="row">
                        <div className="col-6"> <input type="submit" onClick={() => dispatch('prev')} className="btn mt-3 btn-warning form-control" value="Prev"></input>
                        </div>  <div className="col-6">  <input type="submit" onClick={() => dispatch('next')} className="btn mt-3 btn-warning form-control" value="Next"></input>
                        </div> </div>
                    <p className="mt-2 fs-6"> By creating an account or logging in, you agree to KMK's Conditions of Use and Privacy Policy.</p>
    ...
    const [render, dispatch] = useReducer(reducer, init)
